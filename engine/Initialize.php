...
class Initialize {
	...
	public function __construct( \Composer\Autoload\ClassLoader $composer ) {
		$this->content  = new \I_Refer\Engine\Context;
		...
		$this->load_classes();
        $this->i_refer_payment_gateway_init();
	}

	/**
	 * Register the I-Refer payment gateway with WooCommerce.
	 *
	 * @since 2.0.0
	 * @return void
	 */
    private function i_refer_payment_gateway_init() {
        add_filter( 'woocommerce_payment_gateways', function( $gateways ) {
            $gateway_class = 'I_Refer\Integrations\WC_Gateway_IRefer';
            if (class_exists($gateway_class)) {
                $gateways[] = $gateway_class;
            } else {
                // Optionally, log an error or take other actions if the class doesn't exist
                error_log('The class ' . $gateway_class . ' could not be found and has not been added to the WooCommerce payment gateways.');
            }
            return $gateways;
        });
    }
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'i_refer_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'i_refer_initialize_failed', $err );
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'I_Refer\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'I_Refer\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'Plugin Name is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', 'i-refer' ) );
			}
